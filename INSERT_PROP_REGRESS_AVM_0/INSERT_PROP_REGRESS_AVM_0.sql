...
AND
(SELECT COUNT(*) FROM LPLIST_AVM 
WHERE UWDATE>=@UWDATE_START AND UWDATE<=@UWDATE_END   
	AND ( NCREIFMETRO=@MDRA 
		OR NCREIFDIVISION=@MDRA 
		OR NCREIFREGION=@MDRA 
		OR @MDRA='ALLUS') 
	AND ( PropertySubType_ASSIGNED=@SUBTYPE 
		  OR @SUBTYPE='ALL' 
		  OR ((PropertySubType_ASSIGNED='GARDEN' OR PropertySubType_ASSIGNED='MIDRISE') 
				AND @SUBTYPE='GARDEN_MIDRISE') 
		OR ((PropertySubType_ASSIGNED='MIDRISE' OR PropertySubType_ASSIGNED='HIRISE') 
				AND @SUBTYPE='MIDRISE_HIRISE') 
		OR ((PropertySubType_ASSIGNED='GARDEN' OR PropertySubType_ASSIGNED='MIDRISE' 
				OR PropertySubType_ASSIGNED='HIRISE') 
				AND @SUBTYPE='GARDEN_MIDRISE_HIRISE') 
		OR ((PropertySubType_ASSIGNED='GARDEN(SB)' OR PropertySubType_ASSIGNED='GARDEN') 
			AND @SUBTYPE='GARDEN(ALL)') 
			OR ((PropertySubType_ASSIGNED='MIDRISE(SB)' OR PropertySubType_ASSIGNED='MIDRISE') 
			AND @SUBTYPE='MIDRISE(ALL)') 
		OR ((PropertySubType_ASSIGNED='GARDEN(SB)' OR PropertySubType_ASSIGNED='MIDRISE(SB)') 
				AND @SUBTYPE='GARDEN_MIDRISE(SB)')
		)
	AND (DATANUM=0 OR DATANUM=1 OR DATANUM=2 OR DATANUM=3)
AND
CASE 
	WHEN @DEPVAR='UW_CAPRATE' THEN UW_CAPRATE 
	ELSE 
		CASE WHEN @DEPVAR='UW_REV_ACTUAL_RATIO' THEN UW_REV_ACTUAL_RATIO
		ELSE
			CASE WHEN @DEPVAR='UW_EXP_ACTUAL_RATIO' THEN UW_EXP_ACTUAL_RATIO 
		END 
	END 
END IS NOT NULL)
...
AND 
	(
		NCREIFMETRO=@MDRA OR 
		NCREIFDIVISION=@MDRA OR 
		NCREIFREGION=@MDRA OR 
		@MDRA='ALLUS'
	) AND
	(
		PropertySubType_ASSIGNED=@SUBTYPE OR 
		@SUBTYPE='ALL' OR 
		((PropertySubType_ASSIGNED='GARDEN' OR PropertySubType_ASSIGNED='MIDRISE') AND @SUBTYPE='GARDEN_MIDRISE') OR 
		((PropertySubType_ASSIGNED='MIDRISE' OR PropertySubType_ASSIGNED='HIRISE') AND @SUBTYPE='MIDRISE_HIRISE') OR 
		((PropertySubType_ASSIGNED='GARDEN' OR PropertySubType_ASSIGNED='MIDRISE' OR PropertySubType_ASSIGNED='HIRISE') AND @SUBTYPE='GARDEN_MIDRISE_HIRISE') OR
		((PropertySubType_ASSIGNED='GARDEN(SB)' OR PropertySubType_ASSIGNED='GARDEN') AND @SUBTYPE='GARDEN(ALL)') OR 
		((PropertySubType_ASSIGNED='MIDRISE(SB)' OR PropertySubType_ASSIGNED='MIDRISE') AND @SUBTYPE='MIDRISE(ALL)') OR 
		((PropertySubType_ASSIGNED='GARDEN(SB)' OR PropertySubType_ASSIGNED='MIDRISE(SB)') AND @SUBTYPE='GARDEN_MIDRISE(SB)'))
AND (DATANUM=0 OR DATANUM=1 OR DATANUM=2 OR DATANUM=3)
AND
